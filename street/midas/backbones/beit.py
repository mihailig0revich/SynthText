...
def block_forward(self, x, resolution, shared_rel_pos_bias: Optional[torch.Tensor] = None):
    """
    Robust Block.forward: поддерживает и drop_path, и drop_path1 (встречаются в разных версиях timm).
    """
    # выберем правильный drop_path
    dp = getattr(self, "drop_path", None)
    if dp is None:
        dp = getattr(self, "drop_path1", None)
    if dp is None:
        # запасной вариант: пасс-тру (без стохастического пути)
        def _id(y): return y
        dp = _id

    if self.gamma_1 is None:
        x = x + dp(self.attn(self.norm1(x), resolution, shared_rel_pos_bias=shared_rel_pos_bias))
        x = x + dp(self.mlp(self.norm2(x)))
    else:
        x = x + dp(self.gamma_1 * self.attn(self.norm1(x), resolution,
                                            shared_rel_pos_bias=shared_rel_pos_bias))
        x = x + dp(self.gamma_2 * self.mlp(self.norm2(x)))
    return x
